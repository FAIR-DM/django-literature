...
Alongside a reference's own fields, the form the front end serves carries its dates — issued,
accessed, submitted and the rest of CSL JSON's six date-variable slots — as part of the same page.
...
precisions — a year-only start and a full end date are both valid together. An end date given
...
the page is rejected — an invalid date, a duplicated slot, anywhere else on the form — none of it is
stored, dates included, and the page comes back with everything you entered still in it.
