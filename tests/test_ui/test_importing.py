...
class TestImportReport:
    ...
    def test_citation_key_present_where_the_entry_has_a_handle(self):
        result = ImportResult(
            entries=[EntryResult(outcome=Outcome.SKIPPED, index=0, handle="Doe2024")]
        )
        row = ImportReport(result).rows[0]
    ...
    def test_reason_present_only_on_failures(self):
        result = ImportResult(
            entries=[
                EntryResult(
                    outcome=Outcome.FAILED, index=0, reason="could not be parsed"
                ),
    ...
    def test_a_created_row_carries_the_url_of_its_item(self):
        item = ItemFactory()
        result = ImportResult(
            entries=[EntryResult(outcome=Outcome.CREATED, index=0, item=item)]
        )
        row = ImportReport(result).rows[0]
