...
- [x] T002 Add `bibtexparser>=1.4.4,<2` to `[project.dependencies]` in `pyproject.toml` and refresh the lock file. Do **not** declare `pyparsing`: it is `bibtexparser`'s own requirement and this package never imports it, so declaring it makes `deptry` report a defined-but-unused dependency (Article VII). `deptry` goes green once T006 imports the library.
