...
- [ ] T001 Create `tests/test_importers/test_bibtex/__init__.py` and the fixture directory `tests/fixtures/bibtex/`, mirroring the source tree per the testing standard.
...
- [ ] T005 Write `tests/test_importers/test_bibtex/test_registration.py`: `BibTeXFormat` is importable from the `literature` namespace (Article X), appears in `available_formats()` with no configuration because it is in the shipped defaults (FR-003), and `get_format("bibtex")` returns it.
...
- [ ] T007 [P] [US1] Write `tests/test_importers/test_bibtex/test_types.py`: every classic entry type maps to its CSL item type, and an unrecognised type maps to `document` rather than failing the entry (FR-006).
- [ ] T008 [P] [US1] Write `tests/test_importers/test_bibtex/test_fields.py`: each classic field maps to its CSL variable (FR-007).
- [ ] T009 [P] [US1] Write `tests/test_importers/test_bibtex/test_names.py`: contributor lists keep source order and role; `von` particles, `Jr` suffixes and `Last, First` forms land in the right parts of `Name`; a brace-wrapped institutional name goes to `literal` unsplit (FR-008, FR-009).
- [ ] T010 [P] [US1] Write `tests/test_importers/test_bibtex/test_dates.py`: a year alone stores year precision, a year and month store month precision, and neither pads a component the source did not state (FR-010).
- [ ] T011 [P] [US1] Write `tests/test_importers/test_bibtex/test_identifiers.py`: `doi`, `isbn`, `issn` and `url` become typed identifier records (FR-011).
- [ ] T012 [P] [US1] Write `tests/test_importers/test_bibtex/test_handles.py`: the cite key becomes the item's citation key and is the handle on every entry result (FR-012).
- [ ] T013 [P] [US1] Write `tests/test_importers/test_bibtex/test_blocks.py`: `@string` macros are expanded in the entries referencing them (FR-013); `@comment` and `@preamble` are reported as skipped, not failed, and create no item (FR-014); a field repeated inside one entry resolves deterministically (FR-016).
- [ ] T014 [P] [US1] Write `tests/test_importers/test_bibtex/test_crossref.py`: an entry inherits its parent's fields where it states none of its own, including where the parent appears later in the file, and the child is still reported at its own position; a cycle terminates and is reported rather than hanging (FR-015).
- [ ] T015 [US1] Create `literature/importers/_bibtex_maps.py` with the entry-type table and the field table, each entry annotated with the dialect it comes from. Classic BibTeX only at this stage; US3 extends both. T007 and T008 go green.
...
- [ ] T021 [P] [US2] Write `tests/test_importers/test_bibtex/test_clean.py`: a DOI written as a resolver URL or carrying a `doi:` label normalizes to the bare identifier (FR-017); LaTeX escapes decode to the characters they represent and capitalization-protecting braces are removed, while an unrecognised construct is left as it stands rather than dropped (FR-018).
- [ ] T022 [P] [US2] Write `tests/test_importers/test_bibtex/test_recovery.py`: a value that cannot be normalized into something the catalogue accepts is preserved instead of failing its entry (FR-019); an unresolvable date goes to the record's existing fallback (FR-020); an entry is reported as failed only where it cannot be parsed or the catalogue rejects it after recovery (FR-021).
- [ ] T023 [US2] Create `literature/importers/_bibtex_clean.py`: LaTeX decoding through `latexenc.latex_to_unicode`, brace stripping, and per-field normalization for DOI and ISBN. Nothing in it evaluates its input (FR-029). T021 goes green.
...
- [ ] T026 [P] [US3] Write `tests/test_importers/test_bibtex/test_biblatex.py`: `journaltitle` lands as container title exactly as `journal` does; a single `date` field stores at the precision it states; BibLaTeX-only entry types map to their CSL types rather than to `document`; a file mixing both conventions reads correctly (FR-022, FR-023).
- [ ] T027 [P] [US3] Write `tests/test_importers/test_bibtex/test_precedence.py`: an entry carrying both `date` and `year` that disagree resolves deterministically, in the documented direction (FR-024).
- [ ] T028 [US3] Extend both tables in `_bibtex_maps.py` to BibLaTeX types and fields, keeping the dialect annotation. T026 goes green.
...
- [ ] T034 [P] Generate `docs/bibtex-mapping.md` from the tables in `_bibtex_maps.py` so the published mapping cannot drift from the code, and add a test asserting the two agree (FR-007).
...
- [ ] T036 [P] Write `tests/test_importers/test_bibtex/test_containment.py`: `bibtexparser` is imported by `literature/importers/bibtex.py` and by no other module, so the parser stays replaceable.
