...
- [x] ~~T009 [US1] Write `demo/management/commands/demo.py`~~ — **removed 2026-08-12** at the maintainer's instruction (decisions.md D14). The documented path is `migrate`, `seed_demo`, `runserver`. The missing-`ui`-extra guarantee it carried lives in `demo/settings.py` and is now checked against `migrate`.
