...
- [ ] T009 [US1] Write `demo/management/commands/demo.py`: call `migrate`, then `seed_demo`, then `runserver` **with the autoreloader off (`use_reloader=False`)**, printing the address to open. Without that, `runserver` relaunches `manage.py demo` verbatim in a child process, so the destructive re-seed runs again at every start and again on every file save — wiping anything entered through the admin the demo keeps mounted. It must fail with a plain message naming the missing extra when `literature.ui` is not installed, rather than dying inside Django's app loading (spec edge case).
...
- [ ] T016 [US3] Add `.github/workflows/demo.yml`: install with `--extras ui`, then **start the demo by running the documented `python manage.py demo` in the background** — not by composing `migrate`, `seed_demo` and a server start itself, which would leave the single command FR-003, SC-001 and SC-002 are about covered by no check at all. Poll a bounded readiness check rather than sleeping, run `demo/smoke.py`, and stop the server. T009's `--noreload` is what makes the backgrounded command a single process the workflow can poll and stop. The workflow triggers on `pull_request` (never `pull_request_target`), declares `permissions: contents: read`, and inherits no secrets — it needs none, installing from the public index and asserting locally, and unlike `tests.yml` and `build.yml` it runs the pull request head's own code. Filter paths on `push`; do **not** filter on `pull_request`, and carry the comment explaining why, as `tests.yml` and `build.yml` do (FR-022).
