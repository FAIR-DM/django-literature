...
- [ ] T024 Rename `Format` to `BibFormat` throughout — class, imports, `__all__`, tests, docstrings, `CONTEXT.md`, README, CHANGELOG, and the spec artifacts that name it. Mechanical, but it must be complete: a stale `Format` in a docstring is the kind of thing that outlives three releases.
