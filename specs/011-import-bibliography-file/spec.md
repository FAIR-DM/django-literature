...
**Refined again**: 2026-08-24 — the preview becomes a page in its own right, with its own address, and loses the form. Sam, in session, after using the preview. See FR-045 to FR-055 and `decisions.md` D30, as amended the same day by D31: a carried-out import returns the reader to the catalogue rather than to a page of its own. `plan.md` and `tasks.md` carry the cascade.
...
8. **Given** a narrowed table, **When** the counts above it are read, **Then** they still describe the whole file rather than the rows on screen.
9. **Given** the preview page, **When** its foot is read, **Then** one row carries exactly three controls: back to the catalogue, restart, and confirm.
10. **Given** the preview page, **When** restart is chosen, **Then** the staged file is discarded and the reader is returned to an empty import form.
11. **Given** the preview page, **When** confirm is chosen, **Then** the browser ends at the catalogue rather than rendering the result in place, and a message there states what was created.
12. **Given** the preview address, **When** it is reached with nothing staged, **Then** the page says so plainly and does not raise.
13. **Given** the preview page, **When** it is reloaded, **Then** it shows the same preview and imports nothing.
14. **Given** any page this feature adds, **When** its breadcrumb is read, **Then** the step back to the catalogue is a link and carries the catalogue's own title.
...
- **FR-011**: A submitted file MUST produce an import report naming the outcome of every entry, and the reader MUST have read it before anything is written. *(Amended 2026-08-24. The original wording — "a completed import MUST render an import report, and MUST NOT redirect to the catalogue" — was written when the import happened first and the report described it afterwards. With the preview step the report comes first, so the redirect it forbade is now the right ending: the reader has already read the detail, and what they want next is their catalogue. See FR-052 and `decisions.md` D31.)*
...
- **FR-049a**: While the table is narrowed, the counts above it MUST continue to describe the whole file, so that a hidden row cannot be read as a missing one.
...
- **FR-052**: A carried-out import MUST redirect to the catalogue, carrying a message that states what was created. *(Amended 2026-08-24 from "MUST redirect to a success address" — see `decisions.md` D31.)*
- ~~**FR-053**: The success page MUST state what was created, and MUST offer two controls: back to the catalogue, and import another file.~~ **Reversed 2026-08-24 — there is no success page. See FR-052 and `decisions.md` D31.**
- **FR-054**: Reaching the preview address with nothing staged MUST say so plainly rather than raising or rendering an empty page.
