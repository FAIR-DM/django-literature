...
- **FR-020**: Identifier values stored in `ItemIdentifier` MUST be validated at the model layer for well-known types. DOI values MUST match the pattern `10.\d{4,}/\S+`. ISBN values MUST be a valid ISBN-10 or ISBN-13 (digit count and check-digit verification). ISSN values MUST match the pattern `\d{4}-\d{3}[\dX]`. URL values MUST be a syntactically valid absolute URL (scheme + authority). PMID values MUST be numeric strings. PMCID values MUST be either the canonical `PMC`-prefixed digit string or a bare digit string. Validation MUST run on every write that passes through `ItemIdentifier.save()`, and MUST also run through `clean()` so forms and `full_clean()` callers inherit it. It MUST raise `ValidationError` with a descriptive, translated (`gettext_lazy`) message. Unknown identifier types have no format constraint and MUST be stored without format validation.

  *Corrected 2026-08-03.* This requirement previously specified Django validators attached to the `ItemIdentifier.value` field, and required PMCID values to be numeric. Neither could hold. A field validator receives only the value, so it cannot dispatch on the sibling `type` field, and attaching one to `value` would apply the same rule to every identifier type. The numeric-only PMCID rule also contradicted the canonical NCBI form (`PMC2728067`), which the package's own model tests already stored.
...
- **SC-006**: Model-layer format validation rejects malformed DOI, ISBN, ISSN, URL, PMID, and PMCID values with a `ValidationError`, on `save()` as well as through `full_clean()`. Unknown identifier types are stored without format validation. This satisfies the Constitution Principle I requirement that "invalid identifiers MUST NOT be silently stored."
